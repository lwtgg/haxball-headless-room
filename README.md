...
with no human admin needed to keep it running.

**Stack:** Node.js 22 (CommonJS), `haxball.js`, pm2 on a Linux VPS. No browser,
no Puppeteer. Roughly 60–100 MB of RAM per room.

**Scope:** ~7,400 lines across 13 modules, with 5 automated test suites that run
against a fake Haxball API so the whole match flow can be tested without a token
or a live connection.

### What it does

- **Resizes itself.** Picks training, 1v1, 2v2, 3v3 or 4v4 from the number of
  players present and swaps the pitch to match, growing a live match in pairs
  without stopping play wherever the map allows it.
- **Runs the queue.** Winner-stays rotation, captain picks by number, automatic
  substitutions, and `!ceder` to give your seat away.
- **Detects AFK by movement, not keystrokes.** Samples each player's disc
  position once a second, because Haxball only fires activity events on key
  *press* and was kicking players mid-sprint.
- **Protects itself.** Strips admin from anyone outside the staff list, including
  when the engine hands it out automatically, and reverts map changes, team
  unlocks and rate-limit tampering. Bans and mutes are keyed on auth hash rather
  than session id so they survive restarts.
- **Keeps persistent progression.** Stats, XP, ranks, coins, a cosmetics store
  and match betting, all keyed on the player's Haxball auth so they survive
  restarts and nickname changes.
- **Self-heals.** A reconcile loop re-derives the correct room state every four
  seconds rather than trusting any single event to fire, every callback and
  timer is wrapped, and a heartbeat verifies against Haxball's own room list
  instead of a local array.
- **Bilingual.** Spanish primary with English command aliases.

### Running it

```bash
npm install && npm test    # full suite, no token needed
npm start
```

Full setup, configuration and operational notes follow.
